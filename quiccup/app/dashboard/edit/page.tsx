...
import { SectionEditor } from '../websites/[id]/edit/SectionEditor'
import { FinalProduct } from '../websites/[id]/edit/FinalProduct'
import { 
  ChevronLeftIcon, 
...
import { WebsiteData } from '../websites/[id]/edit/types'
...
export default function EditWebsitePage() {
  ...
  // Load initial data - now using user ID directly instead of website ID
  useEffect(() => {
    async function loadWebsiteData() {
      ...
      try {
        ...
        // Load website data using user_id instead of website_id
        ...
        // Load menu items from dedicated table using user_id
        const { data: menuItems, error: menuError } = await supabase
          .from('menu_items')
          .select('*')
          .eq('user_id', user.id) // Changed from website_id to user_id
  ...
  // Function to save menu items to the dedicated table - now using user_id
    ...
    try {
      ...
      if (menuItems.length > 0) {
        const itemsToInsert = menuItems.map(item => ({
          user_id: user.id, // Changed from website_id to user_id
          name: item.title,
                ...
                <div className="flex flex-col h-full">
                  ...
                  <div className="px-2 flex-1">
                    ...
                    <SectionEditor 
                      ...
                      websiteId={user?.id || ''} // Pass user ID instead of website ID
            ...
            <div className="p-4 border-b flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() => router.push(`/dashboard`)} // Changed from website-specific URL
                  className="text-gray-600"
                >
